    ...
    React.useEffect(() => {
        api.getCampaigns().then((json) => {
            ...
            intervalRef.current = window.setInterval(() => {
                setActiveCampaignIndex((prev) => (prev === json.data.length - 1 ? 0 : prev + 1));
            }, 5000);
        });
    }, []);
    return (
            ...
            {campaigns.map((campaign, index) => (
                ...
                >
                    <div className="carousel__campaign-story">
                        <div className="carousel__campaign-story-content">{campaign.story.split('\r\n').slice(0, 3).join('\r\n')}</div>
                        <div className="carousel__campaign-story-title">{campaign.story.split('\r\n')[3]}</div>
                    </div>
                </a>
            ))}
        ...
        <React.Fragment>
            <div className="products">
                {products.map((product) => (
                    <a className="product" key={product.id} href={`./product.html?id=${product.id}`}>
                        ...
                        <div className="product__colors">
                            {product.colors.map((color) => (
                                <div className="product__color" style={{ backgroundColor: `#${color.code}` }} key={color.code} />
                            ))}
                        </div>
