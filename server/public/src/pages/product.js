        ...
        return (
            <div className="product__color-selector">
                {product.colors.map((color) => (
                    <div
                        key={color.code}
                        className={`product__color${color.code === selectedColorCode ? ' product__color--selected' : ''}`}
            ...
            <div className="product__size-selector">
                {product.sizes.map((size) => {
                    ...
                    return (
                        <div
                            key={size}
                            className={`product__size${size === selectedSize ? ' product__size--selected' : ''}${stock === 0 ? ' product__size--disabled' : ''}`}
                            onClick={() => {
                                if (stock === 0) return;
    ...
    function renderProductQuantitySelector() {
        return (
            <div className="product__quantity-selector">
                <div
                    className={`${!selectedSize || quantity === 1 ? 'product__quantity-disable' : 'product__quantity-minus'}`}
                    onClick={() => {
                ...
                <div
                    className={`${!selectedSize || quantity === stock ? 'product__quantity-disable' : 'product__quantity-add'}`}
                    onClick={() => {
                        if (!selectedSize) return;
